  ...
  function postPackage() 
  {
    if( count( $id = Input::get('id'))<1){
      return Redirect::to( 'instantmodule' )
        ->with('messagetext', 'Nothing to select, please reselect module ')->with('msgstatus','error'); 
  ...
  function postInstall( $id =0)
  {
    ...
    if ($validator->passes()) {
      ...
      SximoHelpers::write_route();
          ...
          return Redirect::to('module')->with('messagetext','Module Installed' . $msg)->with('msgstatus','success');
    }  else  {
         return Redirect::to('module')->with('messagetext','Please select file to upload !')->with('msgstatus','error');
    } 
