...
class ConfigController extends BaseController  {
	...
	static function postSave()
	{
		...
		if ($validator->passes()) {
			...
			$val .= 	"define('CNF_MODE','".(!is_null(Input::get('cnf_mode')) ? 'production' : 'development' )."');\n";										
			$val .= 	"?>";
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('config')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('config')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
	...
	function postEmail()
	{
		...
		if ($validator->passes()) 
		{
			...
			return Redirect::to('config/email')->with('messagetext', 'Email Has Been Updated')->with('msgstatus','success');	
			
		}	else {

			return Redirect::to('config/email')->with('messagetext', 'The following errors occurred')->with('msgstatus','success')
			->withErrors($validator)->withInput();
		}
...
return array(	
	...
	"providers"  => array (
		...
		"Twitter"    => array (
			...
			fwrite($fp,$content); 
			fclose($fp);		
			return Redirect::to('config/security')->with('messagetext','Social Media Has Been Updated')->with('msgstatus','success');
		}	else {

			return Redirect::to('config/security')->with('messagetext', 'The following errors occurred')->with('msgstatus','error')
			->withErrors($validator)->withInput();
		}	
	}		
	...
	public function postLogin()
	{
		...
		if ($validator->passes()) {
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return Redirect::to('config/security')->with('messagetext','Setting Has Been Save Successful')->with('msgstatus','success');
		} else {
			return Redirect::to('config/security')->with('messagetext', 'The following errors occurred')->with('msgstatus','error')
			->withErrors($validator)->withInput();
		}	
	...
	public function getClearlog()
	{
		...
		return Redirect::to('config/log')->with('messagetext','Cache has been cleared !')->with('msgstatus','success');	
	}
	...
	public function postAddtranslation()
	{
		...
		if ($validator->passes()) {
			...
			return Redirect::to('config/translation')->with('messagetect','New Translation has been added !')->with('msgstatus','success');	;			
			
		} else {
			return Redirect::to('config/translation')->with('messagetext','Failed to add translation !' )->with('msgstatus','error')->withErrors($validator)->withInput();
		}		
	...
	public function getRemovetranslation( $folder )
	{
		...
		return Redirect::to('config/translation')->with('messagetext','Translation has been removed !')->with('msgstatus','success');	
