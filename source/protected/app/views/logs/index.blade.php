...
<div class="sbox animated fadeInRight">
	...
	<div class="sbox-content"> 	
	    <div class="toolbar-line ">
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('logs/add?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
			...
			<a href="{{ URL::to('logs/download?md='.$masterdetail["filtermd"].$trackUri) }}" class="tips btn btn-sm btn-white" title="{{ Lang::get('core.btn_download') }}">
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
						...
						<a href="{{ URL::to('logs/show/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_view') }}"><i class="fa  fa-search text-info"></i></a>
						@endif
						@if($access['is_edit'] ==1)
						<a  href="{{ URL::to('logs/add/'.$id.'?md='.$masterdetail["filtermd"].$trackUri)}}" class="tips btn btn-xs btn-white" title="{{ Lang::get('core.btn_edit') }}"><i class="fa fa-edit text-success"></i></a>
						@endif
